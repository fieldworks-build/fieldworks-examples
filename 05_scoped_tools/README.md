...
**Does not touch:** a live model call. This is proven at the config/dispatch layer (deterministic, free, no API key) rather than by having a real agent attempt `force_execute` and get refused. `resolve_tools()`/`merge_call_args()` are the real `fieldworks.aggregator` runtime helpers, not a local stand-in — pure functions that apply `ServerDef.include_tools`/`default_args` to whatever tool list and call args the host aggregator hands them. `SERVER_TOOLS` is still a stand-in for what introspecting the real MCP server would return — live introspection stays the host's job.

**Depends on:** `fieldworks.aggregator` (`load_aggregator_config`, `resolve_tools`, `merge_call_args`).
